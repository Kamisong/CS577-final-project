#This is the version with content and author as input, we delete the sources part.
import pandas as pd
...
tokenizer.fit_on_texts( X_train['authors'] + ' ' + X_train['content'])

X_train_authors = tokenizer.texts_to_sequences( X_train['authors']+' '+X_train['content'])
X_val['authors'] = X_val['authors'].astype('str')
X_val['source'] = X_val['source'].astype('str')
X_val['content'] = X_val['content'].astype('str')
tokenizer.fit_on_texts(X_val['authors'] + ' ' + X_val['content'])
X_val_authors = tokenizer.texts_to_sequences(X_val['authors'] + ' ' + X_val['content'])
...
X_test_authors = tokenizer.texts_to_sequences(X_test['authors'] + ' ' + X_test['content'])

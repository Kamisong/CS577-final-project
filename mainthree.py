#This is the version with only content as input, we delete the authors and source parts.
import pandas as pd
...
X_train_authors = tokenizer.texts_to_sequences(X_train['content'])
X_val['authors'] = X_val['authors'].astype('str')
X_val['source'] = X_val['source'].astype('str')
X_val['content'] = X_val['content'].astype('str')
tokenizer.fit_on_texts(X_val['content'])
X_val_authors = tokenizer.texts_to_sequences( X_val['content'])
...
X_test_authors = tokenizer.texts_to_sequences( X_test['content'])
